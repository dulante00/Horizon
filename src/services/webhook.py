...
class WebhookNotifier:
    ...
    def __init__(self, config: WebhookConfig, console=None):
        self.config = config
        self.display_name = config.display_name.strip() or "Horizon"
        if console is None:
            try:
            ...
            "card": {
                ...
                "header": {
                    "title": {
                        "tag": "plain_text",
                        "content": (
                            f"{self.display_name} {date} 折叠日报"
                            if lang == "zh"
                            else f"{self.display_name} {date} Collapsible Daily"
                        ),
                    },
                    "template": "blue",
                },
    ...
    def build_daily_summary_messages(
        ...
        if delivery == "summary_and_items":
            ...
            overview_message = {
                **base_vars,
                "message_title": (
                    f"{self.display_name} {date} 总览"
                    if lang == "zh"
                    else f"{self.display_name} {date} Overview"
                ),
        ...
        return [
            {
                **base_vars,
                "message_title": (
                    f"{self.display_name} {date} 日报"
                    if lang == "zh"
                    else f"{self.display_name} {date} Daily"
                ),
        ...
        await self.notify(
            {
                ...
                "result": "failed",
                "timestamp": str(int(datetime.now(timezone.utc).timestamp())),
                "message_title": f"{self.display_name} generation failed",
